...
class ETH3D(Dataset):
    ...
    def get_data(
        ...
        intrinsics_orig: np.ndarray = np.zeros((len(ids), 3, 3))  # original intrinsics before resize
        ...
        for id_index, id in enumerate(ids):
            ...
            # Store original intrinsics before resize
            intrinsics_orig[id_index] = intrinsic.copy()
        ...
        batch["extrs"] = torch.from_numpy(extrinsics).float()
        batch["intrs"] = torch.from_numpy(intrinsics_orig).float()  # original intrinsics for evaluation
