...
def main(hydra_cfg: DictConfig):
    ...
    for dataset_idx, dataset_name in enumerate(all_eval_datasets, start=1):
        ...
        for seq_name in tqdm(dataset.sequence_list):
            ...
            elif sample_config.strategy == "stride":
                ids = np.arange(0, seq_num_frames, sample_config.kf_every).tolist()
            elif sample_config.strategy == "stride-half":
                ids = np.arange(0, seq_num_frames // 2, sample_config.kf_every).tolist()
            elif sample_config.strategy == "first":
                assert sample_config.kf_every == 1, "first strategy only supports kf_every=1"
                assert sample_config.num_frames <= seq_num_frames, f"sequence {seq_name} has only {seq_num_frames} frames < {sample_config.num_frames}"
                ids = np.arange(0, sample_config.num_frames).tolist()
            elif sample_config.strategy == "uniform":
                assert sample_config.num_frames <= seq_num_frames, f"sequence {seq_name} has only {seq_num_frames} frames < {sample_config.num_frames}"
                ids = np.linspace(0, seq_num_frames - 1, sample_config.num_frames, dtype=int).tolist()
            else:
